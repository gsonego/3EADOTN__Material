...
Grants yourself Key Vault Secrets Officer — without this, writing the secret below fails despite being Owner, the control-plane/data-plane split again. Get `<vault-resource-id>` via `az keyvault show --name $KV --resource-group $RG --query id --output tsv`:
